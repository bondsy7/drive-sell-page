...
const Profile = () => {
  ...
  const handleShowroomUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !user) return;
    if (!file.type.startsWith('image/')) { toast.error('Bitte nur Bilddateien hochladen'); return; }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-3xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
        ...
        <Tabs defaultValue="firma" className="space-y-5">
          <TabsList className="flex flex-wrap gap-1 h-auto p-1 bg-muted/50 w-full">
            <TabsTrigger value="firma" className="flex items-center gap-1.5 text-xs sm:text-sm flex-1 min-w-0">
              <Building2 className="w-3.5 h-3.5 shrink-0" />
              <span className="truncate">Firma</span>
            </TabsTrigger>
            <TabsTrigger value="kontakt" className="flex items-center gap-1.5 text-xs sm:text-sm flex-1 min-w-0">
              <Phone className="w-3.5 h-3.5 shrink-0" />
              <span className="truncate">Kontakt</span>
            </TabsTrigger>
            <TabsTrigger value="finanzen" className="flex items-center gap-1.5 text-xs sm:text-sm flex-1 min-w-0">
              <CreditCard className="w-3.5 h-3.5 shrink-0" />
              <span className="truncate">Finanzen</span>
            </TabsTrigger>
            <TabsTrigger value="konto" className="flex items-center gap-1.5 text-xs sm:text-sm flex-1 min-w-0">
          ...
          {/* Tab: Firma */}
          <TabsContent value="firma" className="space-y-5 mt-0">
            <Section icon={<Image className="w-4 h-4" />} title="Firmenlogo">
              <div className="flex items-center gap-4 sm:gap-6 flex-wrap">
                {profile.logo_url ? (
                  <div className="relative group">
                    <img src={profile.logo_url} alt="Logo" className="h-16 sm:h-20 max-w-[160px] sm:max-w-[200px] object-contain rounded-lg border border-border bg-muted p-2" />
                  ...
                  <div className="h-16 sm:h-20 w-[160px] sm:w-[200px] border-2 border-dashed border-border rounded-lg flex items-center justify-center text-muted-foreground text-xs">
                    Kein Logo
                  </div>
                )}
            ...
            <Section icon={<Image className="w-4 h-4" />} title="Showroom-Hintergrund">
              ...
              <div className="flex items-center gap-4 sm:gap-6 flex-wrap">
                {profile.custom_showroom_url ? (
                  <div className="relative group">
                    <img src={profile.custom_showroom_url} alt="Showroom" className="h-20 sm:h-24 max-w-[240px] sm:max-w-[300px] object-cover rounded-lg border border-border" />
                  ...
                  <div className="h-20 sm:h-24 w-[240px] sm:w-[300px] border-2 border-dashed border-border rounded-lg flex items-center justify-center text-muted-foreground text-xs">
                    Kein Showroom-Bild
                  </div>
                )}
                <div className="space-y-2">
                  <input ref={showroomInputRef} type="file" accept="image/*" className="hidden" onChange={handleShowroomUpload} />
          ...
          {/* Tab: Kontakt & Social */}
          <TabsContent value="kontakt" className="space-y-5 mt-0">
            ...
            <Section icon={<Share2 className="w-4 h-4" />} title="Social Media">
            ...
            <Section icon={<Zap className="w-4 h-4" />} title="Credits & Verlauf">
              <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-4">
                <div className="bg-muted/50 rounded-lg p-3 sm:p-4 text-center">
                  <div className="text-xl sm:text-2xl font-bold text-foreground">{creditsLoading ? '...' : balance}</div>
                  <div className="text-[10px] sm:text-xs text-muted-foreground mt-1">Verfügbar</div>
                </div>
                <div className="bg-muted/50 rounded-lg p-3 sm:p-4 text-center">
                  <div className="text-xl sm:text-2xl font-bold text-foreground">{creditsLoading ? '...' : lifetimeUsed}</div>
                  <div className="text-[10px] sm:text-xs text-muted-foreground mt-1">Verbraucht</div>
                </div>
                <div className="bg-muted/50 rounded-lg p-3 sm:p-4 text-center">
                  <div className="text-xl sm:text-2xl font-bold text-foreground">{transactions.length}</div>
                  <div className="text-[10px] sm:text-xs text-muted-foreground mt-1">Transaktionen</div>
                </div>
              </div>
          ...
          {/* Tab: Konto */}
          <TabsContent value="konto" className="space-y-5 mt-0">
