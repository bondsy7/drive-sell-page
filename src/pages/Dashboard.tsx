...
import { Plus, Image, FileText, Download, ExternalLink, Trash2, LogOut, User, MessageSquare, Mail, Phone, Calculator, Receipt, Plug, ShieldCheck, ChevronDown } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
    ...
    <div className="min-h-screen bg-background">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" className="gap-1.5 hidden sm:inline-flex">
                  <Calculator className="w-3.5 h-3.5" /> <span className="hidden md:inline">Rechner</span> <ChevronDown className="w-3 h-3" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end">
                <DropdownMenuItem asChild>
                  <Link to="/leasing-rechner" className="flex items-center gap-2"><Calculator className="w-4 h-4" /> Leasing-Rechner</Link>
                </DropdownMenuItem>
                <DropdownMenuItem asChild>
                  <Link to="/finanzierungsrechner" className="flex items-center gap-2"><Calculator className="w-4 h-4" /> Finanzierungsrechner</Link>
                </DropdownMenuItem>
                <DropdownMenuItem asChild>
                  <Link to="/kfz-steuer-rechner" className="flex items-center gap-2"><Receipt className="w-4 h-4" /> Kfz-Steuer-Rechner</Link>
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
      ...
      <main className="max-w-6xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
        ...
        <div className="flex gap-2 mb-4 overflow-x-auto pb-1 sm:hidden">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="gap-1 text-xs whitespace-nowrap"><Calculator className="w-3 h-3" /> Rechner <ChevronDown className="w-3 h-3" /></Button>
            </DropdownMenuTrigger>
