...
import {
  Plus, Calculator, Receipt, User, Plug, ShieldCheck, LogOut, ChevronDown, CreditCard,
} from 'lucide-react';
import logoLight from '@/assets/logo-light.png';
import CreditBadge from '@/components/CreditBadge';

interface AppHeaderProps {
  /** Optional left-side actions (e.g. Save button) rendered before "Neues Projekt" */
  leftActions?: React.ReactNode;
  /** Use dark bg (primary) instead of card */
...
export default function AppHeader({ leftActions, variant = 'card' }: AppHeaderProps) {
  ...
  const iconClass = isCard ? 'w-4 h-4' : 'w-4 h-4';
    ...
    <header className={`${headerBg} sticky top-0 z-50`}>
      <div className="max-w-7xl mx-auto px-3 sm:px-4 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 sm:gap-1.5">
          {/* Page-specific left actions (e.g. Save) */}
          {leftActions}
