...
const baseInput: PromptAssemblyInput = {
  ...
  scene: { scenePackId: "pack-1", scenePlateId: "plate-9" },
  logo: { logoAssetId: "logo-7" },
};
...
describe("prompt assembler structure", () => {
  ...
  it("lists references with the primary first and scene/logo rules", () => {
    const { sections } = assembleStrictReferencePrompt(baseInput);
    ...
    expect(manifest).toContain(
      "Scene plate plate-9 from pack pack-1: environment only; it must never change the vehicle.",
    );
    expect(manifest).toContain("never on the vehicle");
    expect(manifest).toContain(
      "Vehicle emblems come exclusively from the reference images.",
    );
  });
});
