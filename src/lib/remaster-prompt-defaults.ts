...
If ANY check fails, regenerate that detail from the reference image.
  ...
  vehicle_scale_lock: {
    ...
    description: 'Regeln für konsistente Fahrzeuggröße (35-45% Bildbreite), tiefere Platzierung, Bodenkontakt und Anti-Weißrand.',
    prompt: `ABSOLUTE SCALE AND POSITION RULES – ZERO DEVIATION BETWEEN IMAGES:
1. CONSISTENT SIZE: The vehicle MUST occupy EXACTLY 35-45% of the image WIDTH in EVERY full-body exterior shot. NOT more, NOT less. The vehicle should appear photographed from a natural distance – never filling the frame.
2. DEPTH PLACEMENT: Place the vehicle several meters INSIDE the showroom, slightly farther back from the camera. There must be generous visible floor space in front of the nearest bumper/tire.
3. VERTICAL CENTER: The vehicle's vertical center (wheel-to-roof midpoint) MUST be at approximately 58% from the top of the image.
4. HORIZONTAL CENTER: The vehicle's center of mass MUST be horizontally centered (50% ± 5%) for symmetric views. For 3/4 views, do NOT push the nearest front/rear corner toward the image edge.
5. GROUND PLANE: ALL four wheels MUST sit on the SAME ground plane and follow the showroom's floor perspective/vanishing point.
6. NO VARIATION: The vehicle must appear the EXACT same physical size across ALL perspectives – front, side, rear, 3/4.
7. BREATHING ROOM: Maintain at least 15% padding between vehicle and image edge on all sides. The scene/showroom MUST be generously visible around the car.
8. PERSPECTIVE CONSISTENCY: Even when camera angle changes, the apparent size must remain constant. Wide-angle distortion and oversized close-up placement are FORBIDDEN.
9. FULL BLEED: The generated image MUST fill the ENTIRE canvas edge-to-edge. There must be ZERO white borders, ZERO blank margins, ZERO unfilled edges. The background/scene MUST extend to every pixel of the image boundary.`,
  },
...
Maintain minimum 5% free space between vehicle edge and image border on all sides.`,
  ...
  scene_lighting_exterior: {
    ...
    prompt: `PROFESSIONAL AUTOMOTIVE LIGHTING INTEGRATION (ZERO TOLERANCE): This is NOT a cut-out, background swap, collage, or pasted vehicle. Re-render the vehicle and room as ONE cohesive professional automotive photograph.
LIGHT-SOURCE PROOF: The final image must visibly explain where the light comes from. Ceiling LEDs, window light, cove lights, streetlights, sun direction, or studio softboxes must create believable highlights on the paint, glass, chrome and floor. The hood, roof and side panels MUST show the NEW ceiling/window/light shapes as natural soft specular reflections.
REFLECTIONS: Completely PURGE and RE-RENDER ALL vehicle reflections to match ONLY the NEW scene. Every reflective surface (paint, windows, mirrors, chrome, headlights, taillights, wheel rims, glossy black trim, sunroof) MUST mirror ONLY the new environment — its walls, floor, ceiling, light fixtures, windows and approved logos. ABSOLUTELY FORBIDDEN in any reflection: trees, sky, clouds, other cars, buildings, people, photographers, asphalt patterns, parking lines, old dealer logos, banners, watermarks, text, or ANY trace of the original photo's environment. The original surroundings MUST NOT be visible anywhere on the vehicle's body, glass, chrome or rims — not even faintly. Through the windows, only the new scene may be visible — never the old environment, never a generic outdoor view. If a door side, rear quarter panel, hood or windshield shows a reflection pattern that does not belong to the target showroom, treat it as contamination and regenerate that surface from scratch.
3/4 VIEW PLACEMENT: For front-right/front-left 3/4 views, the vehicle must be smaller and farther back in the showroom. Keep visible floor between the camera and the nearest bumper/tire; the front corner must not feel too close to the viewer.
NATURAL REFLECTION STRENGTH: Add realistic, subtle reflections — not mirror-perfect CGI. Paint reflections must be soft and curved by the body panels; glass reflections may be sharper; chrome and rims may be brighter but must still show ONLY the new scene.
GROUNDING: Generate realistic contact shadows and ambient occlusion beneath the vehicle. Tires MUST visibly touch the floor. Shadow direction, length and softness MUST match the visible light sources. The floor must show a subtle, natural vehicle reflection if the material is polished or wet.
LIGHTING: Use natural, professional showroom/editorial lighting that matches the scene. Avoid HDR, plastic CGI, exaggerated glow, artificial colour casts, or flat pasted lighting. The result must look like a real professional dealership photo shot in that exact environment.`,
  },
...
- WRONG REFLECTION KILL SWITCH: If any side-panel, hood, windshield or quarter-panel reflection still resembles the original photo environment, completely repaint and re-reflect that surface using only the showroom architecture, ceiling LEDs, windows, green accents and polished floor.
...
export const SCENE_PROMPT_DEFAULTS: Record<string, string> = {
  ...
  'showroom-4': 'The car is positioned in an identical, minimalist, high-tech showroom setting, featuring a highly reflective, dark polished resin floor. The background is composed of clean pastel-colored shapes and soft contours subtly illuminated from behind with a very soft, diffused, cool-white gradient light, creating a sense of infinite depth. The background is perfectly clean, with no extraneous objects, humans, or other vehicles. License plates are blank and body-colored. This exact setting and lighting must be maintained consistently across all generated panels.',
  ...
  'dealer-lot-dekra': 'Empty DEKRA vehicle lot in front of a modern, clean industrial service hall. GROUND: Wide, flat lot paved with light gray-beige interlocking concrete pavers in a subtle diagonal/H-pattern, clean and dry, no puddles, no oil stains, no road markings – IDENTICAL paving pattern in every image and every perspective. BACKGROUND (IMMUTABLE, MUST BE THE SAME IN EVERY IMAGE): A long single-story modern service hall with a bright white / very light gray flat metal panel facade, a darker anthracite-gray vertical section with tall narrow windows on the right, a horizontal band of small dark rectangular clerestory windows above four to five closed anthracite sectional roll-up gates aligned in a row. To the far left of the building a group of green deciduous trees behind a low black metal fence. Clear soft blue sky with a smooth gradient. DEKRA LOGO PLACEMENT (MANDATORY): A single large official DEKRA logo is mounted flush on the bright facade of the hall, high above the roll-up gates, centered horizontally over the gate row. The logo consists of the green DEKRA "play"-triangle mark on the LEFT and the bold uppercase green wordmark "DEKRA" on the RIGHT, both in the exact official DEKRA green (approx. Pantone 348 C, #007a33). Reproduce the logo PIXEL-FOR-PIXEL from the provided reference image – do NOT invent, restyle, recolor, rotate, mirror, stretch, add outlines, add shadows, or add extra text. Only ONE DEKRA logo, no repetition, no additional signage, no additional text anywhere on the building. STRICT: absolutely NO other vehicles, NO cars, NO trucks, NO people, NO other signage, NO other logos, NO other text, NO cones, NO trailers, NO pallets, NO flags – the lot must be completely empty. The same hall, same DEKRA logo in the same position, same pavers, same trees and same sky MUST appear in every generated image and every perspective for full visual consistency.',
};
