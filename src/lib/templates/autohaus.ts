import { withAiDisclosureAlt, buildAiDisclosureImageOverlayHTML } from "@/lib/ai-disclosure";
    ...
    leasingConditionsHTML = `
      <div class="card">
        ...
        <div class="leasing-highlight">
          ...
          <div class="rate-value">${data.finance.monthlyRate}<span style="font-size:.7em;font-weight:500;opacity:.85">/mtl.</span>${rateTypeSuffixHTML(data)}<sup style="font-size:.6rem;vertical-align:super;margin-left:.15em">1</sup></div>
  ...
  <div class="page-wrap">
    ...
    <div class="left-col">
      <div class="gallery-card">
        ...
        ${imageBase64
          ? `<span style="position:relative;display:block"><img id="mainImg" class="gallery-main-img" src="${imageBase64}" alt="${withAiDisclosureAlt(`${data.vehicle.brand} ${data.vehicle.model}`)}"/>${buildAiDisclosureImageOverlayHTML("landing")}</span>`
      ...
      <div class="card price-card">
        ...
        ${!isBuy && data.finance.monthlyRate
          ...
          : `<div class="rate">${data.finance.totalPrice || '–'}</div>
             <p class="list-price">${isBuy ? `Fahrzeugpreis${vatNoteInline(data)}` : ''}</p>`
        }
        <hr class="divider"/>
