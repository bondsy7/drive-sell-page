import { getAiDisclosureText, withAiDisclosureAlt, buildAiDisclosureImageOverlayHTML } from "@/lib/ai-disclosure";
  ...
  <div class="container">
    <div class="main-card">
      <div class="image-side">
        ${imageBase64 ? `<span style="position:relative;display:block"><img id="mainImg" src="${imageBase64}" alt="${withAiDisclosureAlt(`${data.vehicle.brand} ${data.vehicle.model}`)}" />${buildAiDisclosureImageOverlayHTML("landing")}</span>` : `<div style="color:#a0aec0;text-align:center;padding:60px">Kein Bild</div>`}
        ${galleryHTML}
      </div>
      <div class="info-side">
    ...
    <div class="section"><h3>Händler & Kontakt</h3>
      <div class="dealer-grid">
        ...
        <div class="rate-box"><div class="rate-label">${getMonthlyRateLabel(data)}</div><div class="rate-amount">${data.finance.monthlyRate || '–'}<span style="font-size:.7em;font-weight:500;opacity:.85">/mtl.</span>${rateTypeSuffixHTML(data)}</div><div class="rate-period">pro Monat</div>${vatNoteHTML(data, 'font-size:.7rem;color:#9ca3af;margin-top:.3rem;display:block;text-align:center')}</div>
      </div>
    </div>
