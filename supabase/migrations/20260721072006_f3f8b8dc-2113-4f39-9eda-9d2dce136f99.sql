...
BEGIN
  ...
  b_counts AS (
    SELECT
      (split_part(obj.name, '/', 2))::uuid AS v_id,
      COUNT(*)::int AS c
    FROM storage.objects AS obj
    JOIN user_vehicles AS uv ON uv.id = (split_part(obj.name, '/', 2))::uuid
    ...
    GROUP BY split_part(obj.name, '/', 2)
  ),
  p_cover AS (
...
BEGIN
  ...
  b_counts AS (
    SELECT
      ...
      COUNT(*)::int AS c
    FROM storage.objects AS obj
    JOIN page_vehicles AS pv ON pv.id = (split_part(obj.name, '/', 2))::uuid
    WHERE obj.bucket_id = 'banners'
      AND split_part(obj.name, '/', 1) = _uid::text
      AND obj.name LIKE '%.png'
      AND split_part(obj.name, '/', 3) NOT LIKE 'state-%'
